...
def naiiveSolve(problem):
        ...
        while i < 9:
            while j<9:
                if problem[i][j]=='_':
                    ...
                    allNum=allNum-cliques_rows[i]-cliques_columns[j]-cliques_boxes[box_id(i,j)]
                    if len(allNum)==1:
                        added.append(addNumber(list(allNum)[0],i,j,True,True))
                    if len(allNum)==0:#deadend
                        forcedHistory.push(added)
                        return False
    ...
    def forcedRemove(L):
        for elem in L:
            remove(elem[0],elem[1],elem[2])

    
    def addNumber(n,i,j,deed,forced=False):
        if problem[i][j]=='_' and deed==True:#add number
            ...
            remove(triad[0],triad[1],triad[2])
        return
        ...
        if problem[i][j]=='_':#we have to fill this spot
            ...
            while n <=9:#nine possible numbers
                ...
                if n not in cliques_rows[i] and n not in cliques_columns[j] and n not in cliques_boxes[box_id(i,j)]:#filter bad ns
                    ...
                    scout=fillForced(i,j)#fill all forced to avoid unecessary backtrack
                    if not scout:
                        backtrack_counter[0]+=1
                        forcedRemove(forcedHistory.pop())
                        addNumber(0,0,0,False)
                    elif search(nexti, nextj,backtrack_counter):#try it out
                        return True#works!
